...
/**
 * Load a country flag image from a URL.
 * Falls back to a gray rectangle if the load fails.
 */
export async function loadFlagImage(flagUrl: string): Promise<HTMLImageElement> {
  return new Promise((resolve, reject) => {
    const img = new Image()
    img.crossOrigin = 'anonymous'
    img.onload = () => resolve(img)
