...
export function processObstacleCollisions(
  ...
  for (const obs of obstacles) {
    for (const car of cars) {
      if (!obs.localHitBy) obs.localHitBy = new Set()
      if (obs.localHitBy.has(car.code)) continue
      if (car.lane !== obs.lane) continue

      // CAR_DISPLAY_W is 64, so center is car.x + 32.
      // Tighten the visual hit distance to 40 pixels for a clean impact.
      const dx = (car.x + 32) - obs.x
      if (Math.abs(dx) < 40) {
        obs.localHitBy.add(car.code)
